...
export async function dashboardStats() {
  ...
  const pending = referrals.filter((r) => r.status !== "CLOSED").length;
  const overdue = referrals.filter((r) => isOverdue(r.updatedAt, r.status, r.dueAt)).length;
  const completed = referrals.filter((r) => isCompletedJourney(r.status)).length;
...
export async function advanceReferral(opts: {
  ...
  userId?: string;
  note?: string;
  extra?: Prisma.ReferralUpdateInput;
}) {
