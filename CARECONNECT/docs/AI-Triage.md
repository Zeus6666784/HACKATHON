...
The frontend must adhere to the following interaction model:

### 6.1 Request/Response
- **Endpoint**: `POST /api/v1/triage/assess`
- **Payload**: `{ "patientId": "string", "symptoms": "string" }`
- **Response**: The documented JSON schema. If a fallback occurred, the `source` field will be `"FALLBACK"`.

### 6.2 Loading & UX
- **Latency**: AI calls may take 1-3 seconds. Show a loading spinner with the text *"Analyzing symptoms for priority..."*.
- **Fallback Notification**: If `source === "FALLBACK"`, the UI should subtly indicate that a deterministic rule-based assessment was used.

### 6.3 Mandatory Safety Disclaimer
The result card **MUST** display the following disclaimer in a visible, non-removable footer:
> **⚠️ NOTICE**: This AI-generated assessment is for clinical decision support only. It is NOT a medical diagnosis or prescription. All results must be verified by a licensed clinician.
