...
export async function runTriage(input: TriageInput, patientId: string): Promise<TriageResult> {
  try {
    return await post<TriageResult>("/triage/assess", {
      patientId,
      triageInput: input,
    });
  } catch (e: any) {
    console.error("Server triage failed, falling back to rules:", e);
    return ruleTriage(input);
  }
}
